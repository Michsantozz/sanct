...
export function useAgentAudioVisualizerAura(
  ...
  useEffect(() => {
    const isVolumeReactiveState = state === 'speaking' || state === 'listening' || state === 'pre-connect-buffering';
    if (!isVolumeReactiveState) {
      return;
    }

    const clampedVolume = Math.max(0, Math.min(volume, 1.5));
    const baseScale = state === 'speaking' ? 0.2 : 0.24;
    const gain = state === 'speaking' ? 0.2 : 0.16;
    animateScale(baseScale + gain * clampedVolume, { duration: 0 });
  }, [
    state,
    volume,
    animateScale,
